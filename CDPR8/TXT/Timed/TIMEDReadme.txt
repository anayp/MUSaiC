Note Data Files for TEXTURE TIMED 
=================================
TIMED repeats a motif of 'notes', where each motif is defined by rhythm alone, in NOTEDATA.*
The motifs are separated by SKIPTIME.
...
• a pitch set (here, the octatonic scale):
...
*NOTE: The difference between TIMED and TGROUPS (timed-groups) is that in the TGROUPS the rhythm is applied to (determines the onset times of) groups of notes instead of individual ones. In TGROUPS, if GP_SIZE (group-size) is set to 1, the output is equivalent to TIMED.

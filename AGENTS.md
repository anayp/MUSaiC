# Composer Desktop Setup Agents

## Mission
- Download and install the Composer Desktop project in `F:\CDP`, then enable programmatic music creation and control via the CLI.

## Ground Rules
- Work in `F:\CDP`. Keep edits ASCII-only unless required.
...
## Agents
- **Acquisition Engineer**: Locate the Composer Desktop source (repo/archive), request network approval, and download it into `F:\CDP`. Verify integrity (hash/signature if provided).
